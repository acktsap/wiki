...
public class JStreamParallelStreamProcessorCallbackTest {

    private static final Logger logger = getLogger(JStreamParallelStreamProcessorCallbackTest.class);

    public static void main(String[] args) {
        Properties consumerProperties = new Properties();
        consumerProperties.setProperty(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG, "localhost:9092");
        consumerProperties.setProperty(ConsumerConfig.GROUP_ID_CONFIG, UUID.randomUUID().toString());
        consumerProperties.setProperty(ConsumerConfig.AUTO_OFFSET_RESET_CONFIG, "earlist");
        ...
        JStreamParallelStreamProcessor<String, String> parallelStreamProcessor = JStreamParallelStreamProcessor.createJStreamEosStreamProcessor(options);
        // parallelStreamProcessor.subscribe(List.of(TOPIC_NAME)); // 이거 왜 안됨? 일관성 어디?
        kafkaConsumer.subscribe(List.of(TOPIC_NAME));

        parallelStreamProcessor.pollProduceAndStream(pollContext -> {
