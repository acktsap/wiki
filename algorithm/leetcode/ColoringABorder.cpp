...
public:
  ...
  int dy[4] = { 0, 0, -1, 1 };
    ...
    - space: O(n * m)
  */
  vector<vector<int>> colorBorderNotCopy(vector<vector<int>>& grid, int row, int col, int color) {
    vector<vector<bool>> visited = vector<vector<bool>>(grid.size(), vector<bool>(grid[0].size(), false));
    dfsNotCopy(row, col, grid[row][col], color, grid, visited);
    return grid;
  }

  void dfsNotCopy(int row, int col, int baseColor, int changeColor, vector<vector<int>>& grid, vector<vector<bool>>& visited) {
    bool isBorder = false;
    for (int i = 0; i < 4; ++i) {
      int x = row + dx[i];
      int y = col + dy[i];
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      dfsNotCopy(x, y, baseColor, changeColor, grid, visited);
    }
  }
  ...
  /*
    ...
    - space: O(n * m)
  */
  vector<vector<int>> colorBorderWithCopy(vector<vector<int>>& grid, int row, int col, int color) {
    vector<vector<int>> copy = vector<vector<int>>(grid);
    vector<vector<bool>> visited = vector<vector<bool>>(grid.size(), vector<bool>(grid[0].size(), false));
    dfsWithCopy(row, col, grid[row][col], color, grid, copy, visited);
    return copy;
  }

  void dfsWithCopy(int row, int col, int baseColor, int changeColor, const vector<vector<int>>& grid, vector<vector<int>>& markGrid, vector<vector<bool>>& visited) {
    bool isBorder = false;
    for (int i = 0; i < 4; ++i) {
      int x = row + dx[i];
      int y = col + dy[i];
    ...
    for (int i = 0; i < 4; ++i) {
      ...
      dfsWithCopy(x, y, baseColor, changeColor, grid, markGrid, visited);
    }
  }
};
...
int main() {
  ...
  for (auto& parameter : parameters) {
...

    {
      ...
      auto& color = get<3>(parameter);
      auto& expected = get<4>(parameter);
      auto actual = solution.colorBorderWithCopy(grid, row, col, color);
      if (actual != expected) {
        cout << "Expected" << endl << expected;
        cout << "Actual" << endl << actual;
        assert (false);
