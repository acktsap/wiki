...
- [Logical Design](#Logical-Design)
  ...
  - [Entity Relation Model (ER Model)](#Entity-Relation-Model)
...
데이터베이스가 존재하기 이전에는 파일 시스템을 이용하여 데이터를 관리하였다. 파일 시스템을 이용해서 데이터를 저장하면 데이터를 각각의 파일 단위로 저장하며 이러한 일들을 처리하기 위한 독립적인 애플리케이션과 상호 연동이 되어야 한다. 이 경우 데이터 종속성 문제와 중복성, 데이터 무결성 등의 문제가 발생한다.
