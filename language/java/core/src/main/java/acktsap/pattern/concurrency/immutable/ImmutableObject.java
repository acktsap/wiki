...
package acktsap.pattern.concurrency.immutable;
        ...
        final Runnable modifyRun = () -> {
            for (int i = 1; i <= tryCount; ++i) {
                ...
                final ImmutableRGB newRgb =
                    color.with(colorSupplier.get(), colorSupplier.get(), colorSupplier.get(),
                        expected);
                actual = newRgb.getName();

                if (!expected.equals(actual)) {
                    System.err.format(
                        "Actual name is different with expected one (Thread: %s, try: %d)\n",
                        Thread.currentThread().getName(), i);
                    return;
                }
            }
            System.out
                .format("Operation finished (Thread: %s)\n", Thread.currentThread().getName());
        };
