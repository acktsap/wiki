...
fun main() {
    ...
    Spec("Return at anonymous function") {
    ...
    Pattern("Use return to break loop") {
        run loop@{
            listOf(1, 2, 3, 4, 5).forEach {
                if (it == 3) {
                    return@loop
    ...
    Pattern("Return function lambda") {
        fun foo() {
            (1..5).forEach {
                if (it == 3) {
